...
query= query + "(artist text,song text,firstname text,lastname text,itemInSession int,userId int,sessionId int,PRIMARY KEY ((userId, sessionId), itemInSession))"
try:
    session.execute(query)
except Exception as e:
...
with open(file, encoding='utf8') as f:
    ...
    for line in csvreader:
        query = """
        INSERT INTO artist_sortedsong_and_users_table (artist, song, firstname, lastname, itemInSession, userId, sessionId)
...
# #### Explanation:
# #### Partition Key: song – Allows us to efficiently query by song title, which is the main filter criterion.
# #### Clustering Keys: userId, sessionId, itemInSession – Ensure uniqueness and ordering within each song partition, allowing retrieval of users who listened to a particular song.
